...
public class CassandraWriter implements Closeable {
	...
	private static final String CREATE_TABLE_IF_EXIST = "CREATE TABLE IF NOT EXISTS revision (id uuid, revision_id int, revision_timestamp timestamp, "
	        ...
	        + "revision_isminor boolean, revision_redirection text, revision_text text, lucene text,"
	        + "primary key (id));";
	...
	public void write(Revision r) {
		...
		try {
			query = QueryBuilder.insertInto("revision")
			                    .values(new String[] { "id", "revision_id", "revision_timestamp", "page_id", "page_ns",
			                                    "page_fulltitle", "page_title", "page_restrictions", "page_isredirect",
			                                    "contributor_id", "contributor_username", "contributor_isanonymous",
			                                    "revision_isminor", "revision_redirection", "revision_text" },
			                            new Object[] { UUID.fromString(new com.eaio.uuid.UUID().toString()), r.getId(),
			                                    r.getTimestamp(), r.getPage().getId(), r.getPage().getNamespace(),
			                                    r.getPage().getFullTitle(), r.getPage().getTitle(),
			                                    r.getPage().getRestrictions(), r.getPage().isRedirect(),
			                                    r.getContributor().getId(), r.getContributor().getUsername(),
			                                    r.getContributor().getIsAnonymous(), r.isMinor(), r.getRedirection(),
			                                    r.getText() })
